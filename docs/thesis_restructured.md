...
| 测试框架 | pytest + httpx |
| 数据库 | SQLite（内存模式） |
| 实验数据 | Amazon Reviews 2023 真实数据集（Health & Personal Care 品类）：624件商品、853名消费者、5026条真实评价、12242条行为记录、783笔订单 |
...
|------|---------|---------|---------|------|
| 推荐接口 | 624商品/853用户/12242行为 | <500ms | <500ms | 通过 |
| 商品列表 | 624商品分页 | <200ms | <200ms | 通过 |
...
基于上述真实数据集，系统计算得到的用户活跃度等级分布为：高活跃79人、普通110人、低活跃664人，三个等级均有分布。在该真实数据上的端到端验证显示，高活跃用户每页获取3条广告、低活跃用户每页仅获取1条广告，差异化频控策略按预期生效。
