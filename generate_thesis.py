...
para(
    ...
    '经77个测试用例验证，系统功能完整、性能达标，能够有效地根据每位用户的个人行为特征与社区数据进行个性化的商品与广告推荐。'
)
...
para(
    ...
    'Validated by 77 test cases, the system can effectively deliver personalized product and ad recommendations tailored to each individual user based on their behavioral characteristics and community data.'
)
...
add_table(
    ...
    [
        ...
        ['实验数据', 'Amazon Reviews 2023 真实数据集（Health & Personal Care）：624商品 / 853消费者 / 5026评价 / 12242行为 / 783订单'],
    ]
)
...
para('系统实验数据取自 Amazon Reviews 2023 真实数据集（Health & Personal Care 品类），经采样得到624件商品、853名消费者及5026条真实评价。在该真实数据上，系统计算出的用户活跃度等级分布为高活跃79人、普通110人、低活跃664人，三个等级均有分布；端到端验证显示高活跃用户每页获取3条广告、低活跃用户每页仅获取1条广告，差异化频控策略按预期生效。')
