...
para('本文共分为七章。第1章绪论，介绍课题的研究背景与意义，综述国内外研究现状，明确研究内容与目标。第2章相关技术与理论基础，详细阐述协同过滤、矩阵分解、DeepFM、DIN、eCPM竞价和时间衰减函数等核心技术原理。第3章需求分析，从电商业务、社区子系统、个性化推荐引擎、个性化广告频控、活跃度引擎和非功能需求六个维度进行需求分析，并专门阐述系统的个性化需求。第4章总体设计，设计系统的整体架构、模块划分、核心数据流、接口、数据库和安全方案。第5章详细设计与实现，阐述数据库设计、推荐引擎、广告系统、频控组件、活跃度引擎和前端界面的设计细节与实现过程。第6章系统测试与分析，制定测试方案并执行单元测试、集成测试和性能测试，分析测试结果。第7章总结与展望，总结研究成果和创新点，分析不足并展望改进方向。')
...
figure_placeholder('图4-3 广告频控流程图')

heading2('4.4 接口设计')

para('系统API遵循RESTful风格，按模块组织为9组路由：')

table_caption('表 4.2 核心API接口设计')
add_table(
    ['模块', '路径前缀', '核心接口'],
    [
