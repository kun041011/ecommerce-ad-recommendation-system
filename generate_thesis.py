...
para('与此同时，电商社区化运营已成为行业趋势。以小红书、淘宝逛逛、京东种草等为代表的电商社区功能，通过商品评价、购物问答、种草分享等形式，不仅增强了用户之间的连接和互动，也为平台积累了丰富的反映用户参与度和忠诚度的行为数据。将这些社区数据纳入广告策略决策，为构建差异化的频控机制提供了新的技术路径。基于以上分析，本文提出了一种基于社区数据反馈的电商广告推荐系统，通过融合电商行为与社区行为构建用户活跃度评分模型，实施分级频控策略，在广告收入与用户留存之间寻求最优平衡。')

heading2('1.2 国内外研究现状')

heading3('1.2.1 推荐系统')

para('推荐系统历经三个发展阶段。第一阶段以协同过滤为代表：1994年GroupLens项目开创了自动化推荐先河，此后UserCF和ItemCF成为经典算法，Samih等人[1]对矩阵分解模型进行了全面评估。第二阶段以矩阵分解为标志：2006年Netflix Prize竞赛推动了SVD、ALS等方法在推荐中的应用，有效缓解了数据稀疏性问题。第三阶段以深度学习为核心：Wang等人提出DCN V2改进特征交叉[2]，Li等人将LSTM与DeepFM结合提升CTR预测[3]。张增杰等[4]提出了基于深度知识图卷积网络的推荐算法，李想等[5]对基于大语言模型的推荐系统进行了系统综述。')

heading3('1.2.2 广告频控')

para('广告频控研究从固定阈值向智能化方向发展。Ogundele等[6]对广告点击率预测算法进行了全面综述，指出传统方法基于固定参数（如每用户每日N次上限），缺乏个性化能力。近年来有研究利用强化学习动态调整展示频率，或从广告疲劳度角度建模用户耐受曲线。赵文婷等[7]研究了社交电商平台用户留存的影响因素，发现社区互动对留存率有显著正向作用。但现有研究主要基于广告上下文中的行为数据，较少将社区互动数据纳入频控决策。本文的创新之处在于将社区行为作为用户活跃度信号引入频控机制。')

heading3('1.2.3 电商社区与用户活跃度')

para('电商社区通过评价、问答、种草等功能提升用户粘性，江积海等[8]研究发现推荐算法对内容平台价值创造具有显著驱动作用。用户活跃度的量化评估从宏观指标（DAU、MAU、留存率）向个体层面发展，通过登录频次、浏览量、操作次数等行为指标加权计算。时间衰减函数<i>f</i>(<i>t</i>)=<i>e</i><sup>-λ<i>t</i></sup>是常用的衰减模型，使近期行为获得更高权重，符合用户兴趣随时间变化的规律。张莹等[9]进一步探索了自适应架构在推荐场景中的应用。')

heading2('1.3 研究内容')

para('本文的研究内容围绕"基于社区数据反馈的电商广告推荐系统"展开，具体包括以下五个方面：1. 系统整体架构设计，将电商业务、社区交互、推荐引擎、广告系统和活跃度引擎整合为统一的分层架构，明确各子系统的职责边界和数据流转路径；2. 多算法融合的推荐引擎，实现召回层（UserCF、ItemCF、Content-Based、ALS、热门召回）、排序层（DeepFM、DIN）和重排层（MMR多样性、业务规则）的完整推荐流水线。')

para('3. 基于社区反馈的活跃度评分模型，融合电商行为（浏览、购买等）和社区行为（评价、问答、点赞）构建加权评分体系，通过时间衰减函数实现动态评估；4. 差异化广告频控机制，根据活跃度等级设计分级频控策略（每页广告数、展示间隔、每日上限），实现广告收入与用户留存的协同优化；5. 完整系统开发与测试，完成前后端全部功能开发和77个测试用例的验证，覆盖单元测试、集成测试和性能测试多个层面。')

heading2('1.4 论文组织结构')

para('本文共分为六章。第一章绪论，介绍课题的研究背景与意义，综述国内外研究现状，明确研究内容与目标。第二章相关技术与理论基础，详细阐述协同过滤、矩阵分解、DeepFM、DIN、eCPM竞价和时间衰减函数等核心技术原理。第三章需求分析与总体设计，从功能需求和非功能需求两个维度进行分析，设计系统的整体架构、模块划分和数据流方案。第四章详细设计与实现，阐述数据库设计、推荐引擎、广告系统、频控组件、活跃度引擎和前端界面的设计细节与实现过程。第五章系统测试与分析，制定测试方案并执行单元测试、集成测试和性能测试，分析测试结果。第六章总结与展望，总结研究成果和创新点，分析不足并展望改进方向。')
...
para('DeepFM由华为诺亚方舟实验室提出，将因子分解机（FM）与深度神经网络（DNN）端到端联合训练，在不需要人工特征工程的前提下同时建模低阶和高阶特征交叉。Li等人[3]在此基础上提出了融合LSTM的改进模型，验证了引入序列信息对CTR预估效果的显著提升。DeepFM架构包含两个并行组件：FM组件负责捕获特征的二阶交叉关系，其中<i>v</i><sub>i</sub>为特征<i>i</i>的隐向量，通过sum-of-square与square-of-sum的差值实现<i>O</i>(<i>kn</i>)复杂度的高效计算，其公式如下：')
...
para('注意力输入为[<i>e</i><sub>i</sub>, <i>e</i><sub>c</sub>, <i>e</i><sub>i</sub>-<i>e</i><sub>c</sub>, <i>e</i><sub>i</sub>⊙<i>e</i><sub>c</sub>]的拼接向量，经两层全连接网络输出注意力得分。这使模型能针对不同候选商品自适应地激活用户历史中的相关兴趣，而非用固定向量表示所有兴趣。')
...
para('在<i>λ</i>=0.1的设置下，各时间点的衰减权重为：今天1.00，3天前0.74，7天前0.50，14天前0.25，21天前0.12，30天前0.05。可以看出，一周前的行为权重已衰减至一半，一个月前的行为基本可以忽略。这意味着活跃度评分主要由用户近1-2周的行为决定，这个时间窗口既足以反映用户的持续活跃趋势，又不会因为过于陈旧的历史行为干扰当前评估。')
...
para('推荐引擎需要同时满足商品推荐和广告推荐两个场景的需求。在商品推荐方面，首页推荐需根据用户历史行为生成个性化推荐列表，新用户使用热门商品兜底，推荐结果中需要按频控策略混排广告；相似推荐需在商品详情页展示与当前商品同品类或特征相似的商品，促进用户浏览深度。在架构方面，系统需要采用多阶段漏斗架构：召回层多路并行快速缩小候选集，排序层用深度模型精细打分，重排层控制多样性并执行业务规则过滤。广告候选集需经过eCPM竞价排序，与推荐商品混排展示时需遵循频控组件的展示约束。')

heading3('3.1.4 广告系统与频控需求')

para('广告系统的核心需求是实现基于活跃度的差异化频控——这是整个系统的创新所在。在广告管理方面，商家角色可创建广告、设置出价（CPC/CPM）、日预算和总预算、定向标签，系统实时追踪广告消耗，预算耗尽后自动暂停投放。在频控策略方面，系统根据用户活跃度等级确定三组差异化参数，具体如下表所示：')
...
figure_placeholder('图3-2 系统整体架构图')
...
figure_placeholder('图3-3 推荐流程图')

figure_placeholder('图3-4 广告频控流程图')
...
figure_placeholder('图3-5 数据库E-R关系图')
...
para('重排层在排序结果的基础上进行最终的结果调整，实现多样性控制和业务规则过滤两个功能。MMR（Maximal Marginal Relevance）多样性重排算法在选择每一个推荐结果时，同时考虑相关性和多样性，每次选择得分最高且与已选集合最不相似的物品，以商品品类作为多样性度量维度，其计算公式如下：')
...
para('RecommendationPipeline类编排完整的推荐流程：调用多路召回→合并去重→排序打分→重排多样性→热门兜底填充→返回Top-N。王鑫等[13]研究了大数据在电商个性化推荐中的应用效果，王洪涛等[14]进一步探索了基于用户行为分析的推荐优化策略。刘海鸥等[15]提出了基于大语言模型的可信推荐框架，为推荐系统的可解释性提供了新思路。对于无历史行为的新用户，直接返回热门召回结果。')
...
para('其中<i>S</i>为活跃度评分，<i>w</i><sub>i</sub>为第<i>i</i>条行为对应的权重值（如购买为10、评价为5、浏览为1等），<i>t</i><sub>i</sub>为该行为距今的天数，min函数确保评分上限不超过100分。classify_activity_level函数根据最终评分划分活跃度等级：得分大于等于60为"high"（高活跃），20到60之间为"normal"（普通），小于20为"low"（低活跃），等级直接映射到频控策略矩阵中的对应参数行。')
...
para('前端共8个页面，按用户角色划分为四类。公共页面包括首页（推荐流加广告混排）、商品详情页（评价、问答、相似推荐和广告位）、搜索页（支持综合、销量和价格排序）以及登录和注册页面。消费者页面包括购物车（数量控制和结算功能）、订单列表和个人中心（包含活跃度仪表盘，以圆形进度条展示当前活跃度评分和等级）。商家页面为商家后台，提供广告创建和管理功能，支持设置出价、预算和定向标签。管理员页面为管理后台，使用ECharts实现数据可视化，包含活跃度分布饼图、广告效果排行柱状图和频控策略效果对比图等数据分析图表。')
...
para('测试结果显示共77个测试用例中73个通过，4个跳过，0个失败。跳过的4个测试用例是由于测试环境中PyTorch库不可用导致的DeepFM和DIN深度模型测试，这些模型已提供numpy替代实现并通过了对应的替代测试覆盖，不影响系统功能的完整性验证。整体测试通过率为100%（不计跳过项），证明系统各模块的功能实现正确、性能指标达标。')
...
figure_placeholder('附图1 系统首页界面')

para('附图1展示了系统首页界面，包含顶部导航栏与搜索框、Hero轮播横幅、品类快速导航、限时特惠商品区域以及推荐商品瀑布流。推荐流中每3个商品穿插1条广告卡片，广告以红色边框和"推广"角标与普通商品区分，页面顶部和底部各有一条全宽横幅广告。')

figure_placeholder('附图2 商品详情页界面')

para('附图2展示了商品详情页，采用左右双栏布局：左侧为商品图片区域，右侧为商品信息面板（名称、价格条、库存、数量选择器、购买和加购按钮）。下方依次为商品评价区（星级评分和文字评价）、商品问答区以及"热门推荐"广告区（3条横排广告卡片）。')

figure_placeholder('附图3 用户个人中心——活跃度仪表盘')

para('附图3展示了用户个人中心页面，核心为活跃度仪表盘组件，以圆形进度条显示当前活跃度评分和对应等级（高活跃/普通/低活跃），下方标注当前用户的广告频率等级，使用户直观了解自己的平台参与程度。')

figure_placeholder('附图4 管理后台数据分析界面')

para('附图4展示了管理后台的数据分析仪表盘，包含四个统计卡片（用户总数、商品总数、订单总数、广告收入）、三个KPI指标（CTR、RPM、交易总额）、用户活跃度分布饼图、广告效果排行混合图（柱状CTR+折线消耗）、频控策略效果对比图以及广告详细数据表格。')

figure_placeholder('附图5 商家后台广告管理界面')

para('附图5展示了商家后台的广告管理页面，商家可在此创建新广告（设置标题、出价、日预算和总预算），并以表格形式查看已创建广告的标题、出价、消耗金额和投放状态（active/paused/exhausted）。')

figure_placeholder('附图6 单元测试执行结果')

para('附图6展示了在终端执行pytest测试套件的输出结果，共77个测试用例，其中73个通过（PASSED）、4个跳过（SKIPPED）、0个失败。跳过的测试为PyTorch环境不可用导致的深度模型测试，已有numpy替代实现覆盖。')
