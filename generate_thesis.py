...
para('电商社区通过评价、问答、种草等功能提升用户粘性，江积海等[6]研究发现推荐算法对内容平台价值创造具有显著驱动作用。用户活跃度的量化评估从宏观指标（DAU、MAU、留存率）向个体层面发展，通过登录频次、浏览量、操作次数等行为指标加权计算。时间衰减函数 f(t)=e^(-λt) 是常用的衰减模型，使近期行为获得更高权重，符合用户兴趣随时间变化的规律。张莹等[11]进一步探索了自适应架构在推荐场景中的应用。')
...
para('DeepFM由华为诺亚方舟实验室提出，将因子分解机（FM）与深度神经网络（DNN）端到端联合训练，在不需要人工特征工程的前提下同时建模低阶和高阶特征交叉。Li等人[2]在此基础上提出了融合LSTM的改进模型，验证了引入序列信息对CTR预估效果的显著提升。DeepFM架构包含两个并行组件：FM组件负责捕获特征的二阶交叉关系，其中v_i为特征i的隐向量，通过sum-of-square与square-of-sum的差值实现O(kn)复杂度的高效计算，其公式如下：')

formula_placeholder('（2-2）', 'y_FM = w₀ + Σᵢ(wᵢxᵢ) + Σᵢ Σⱼ₌ᵢ₊₁ ⟨vᵢ, vⱼ⟩ xᵢxⱼ')

para('Deep组件是多层全连接神经网络，负责捕获特征之间的高阶非线性交叉关系。两个组件共享底层的Embedding层，最终的预测输出为两个组件输出的加和经过Sigmoid激活后的结果。DeepFM的优势在于无需人工特征工程即可自动学习特征交叉，在CTR预估任务上表现优异。Wang等人在此基础上提出了DCN V2[1]，通过引入低秩交叉层进一步改进了特征交叉的建模能力和计算效率。')
