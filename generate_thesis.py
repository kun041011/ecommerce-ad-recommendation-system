...
para('2. 计算用户相似度。采用余弦相似度：sim(u,v) = (r_u · r_v) / (||r_u|| × ||r_v||)，值越大表示越相似。')
...
para('1. FM组件。捕获特征的二阶交叉关系，计算公式为 y_FM = Σ_i Σ_j (v_i · v_j × x_i × x_j)，其中v_i为特征i的隐向量。通过sum-of-square与square-of-sum的差值实现O(kn)复杂度的高效计算。')
...
para('计算广告的核心问题是如何在广告主的出价意愿与用户点击可能性之间找到最优的广告排序方案。业界通用的做法是采用eCPM（effective Cost Per Mille，有效千次展示收入）作为排序指标，其计算公式为：eCPM = bid × pCTR × 1000，其中bid为广告主的出价金额，pCTR为推荐模型预估的广告点击概率。eCPM值越高的广告获得优先展示的机会，这种机制确保了平台在同等展示机会下选择预期收入最高的广告。')

para('系统支持两种计费模式，适应不同广告主的投放需求：')

para('1. CPC（Cost Per Click，按点击付费）。广告主只在用户实际点击广告时才需付费。扣费金额采用GSP（Generalized Second Price，广义第二价格）机制计算：charge = next_eCPM / current_pCTR / 1000 + 0.01，即按排名紧邻的下一位广告的eCPM来确定实际扣费额，加上一个微小增量。GSP机制的优势在于激励广告主如实报价——即使出价较高，实际扣费也只取决于竞争对手的出价水平，从而降低广告主的策略博弈成本。')
...
para('本系统采用指数衰减函数：decay(t) = e^(-λ×t)，其中t表示行为发生距今的天数，λ为衰减率参数，本系统设置为0.1。指数衰减函数具有几个理想的数学性质：首先，函数值域为(0, 1]，今天的行为权重恰好为1.0；其次，衰减速度由λ参数统一控制，便于调优；第三，函数连续可导，权重变化平滑自然。')
...
figure_placeholder('图3-2 推荐引擎数据流程图')

figure_placeholder('图3-3 广告投放与频控流程图')

figure_placeholder('图3-4 活跃度反馈闭环流程图')
...
figure_placeholder('图4-1 多阶段推荐漏斗架构图')
...
para('1. MMR多样性重排。每次选择得分最高且与已选集合最不相似的物品：MMR_score = 0.5×relevance - 0.5×max_similarity。以商品品类为多样性维度。')
...
figure_placeholder('图4-2 频控组件判断流程图')

heading2('4.5 活跃度引擎实现')

para('活跃度评分的核心逻辑在scorer.py中实现。calculate_activity_score函数遍历用户的所有行为记录，对每条记录取行为权重×时间衰减因子并累加，最终取min(100, 总分)作为活跃度评分。')
...
figure_placeholder('图4-3 活跃度评分计算流程图')
