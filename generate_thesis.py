...
para('注意力输入为[<i>e</i><sub>i</sub>, <i>e</i><sub>c</sub>, <i>e</i><sub>i</sub>-<i>e</i><sub>c</sub>, <i>e</i><sub>i</sub>⊙<i>e</i><sub>c</sub>]的拼接向量，经两层全连接网络输出注意力得分。这使模型能针对不同候选商品自适应地激活用户历史中的相关兴趣，而非用固定向量表示所有兴趣。近年来，刘海鸥等[15]进一步提出了基于大语言模型的可信多模态推荐算法，将多模态信息融入推荐模型以增强可解释性，为深度推荐模型的发展提供了新方向。')
